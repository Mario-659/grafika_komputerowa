...
N = 30
RANDOM_COLORS_TAB = numpy.zeros((N, N, 3))
...
def render_egg():
    ...
    for i in range(N - 1):
        for j in range(N - 1):
            ...
            glColor(RANDOM_COLORS_TAB[i, j])
            glVertex(tab[i, j])
            glColor(RANDOM_COLORS_TAB[i, j + 1])
            glVertex(tab[i, j + 1])
            glColor(RANDOM_COLORS_TAB[i + 1, j])
            glVertex(tab[i + 1, j])
            ...
            glColor(RANDOM_COLORS_TAB[i + 1, j])
            glVertex(tab[i + 1, j])
            glColor(RANDOM_COLORS_TAB[i + 1, j + 1])
...
def generateRandomColors():
    for i in range(N):
        for j in range(N):
            RANDOM_COLORS_TAB[i][j] = [random.random(), random.random(), random.random()]
